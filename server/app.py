...
"""    results = response.json()["locations"]
    ...
    for i, location in enumerate(results[:5]):
        if location['code'] is not None:
            formatted_location = f"{location['code']} -- {location['name']} -- {location['country']['name']}"
...
#    from_city = request.args.get('from')
    from_city = "TOY"
    query = '''
    SELECT * FROM "search_history" WHERE "from" = ? ORDER BY "date_time" DESC LIMIT 3;
    '''

    cur.execute(query, (from_city,))
    search_history = cur.fetchall()
